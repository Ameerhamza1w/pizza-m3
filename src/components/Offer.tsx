        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {offers.map((offer, index) => (
            <button
              ...
              className="bg-white text-gray-800 shadow-lg rounded-lg p-6 hover:bg-gray-100 transition-transform duration-300 transform hover:scale-105 relative overflow-hidden"
            >
              {/* Tag for the offer */}
              <div className="absolute top-0 right-0 bg-gradient-to-r from-yellow-500 to-orange-400 text-white text-xs font-bold py-2 px-4 rounded-l-lg shadow-lg">
                {offer.tag}
              </div>

              {/* Offer Title */}
              <h3 className="text-3xl font-semibold mb-4">{offer.title}</h3>
